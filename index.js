...
class JUnitXrayReporter extends MochaJUnitReporter {
	...
	getTestcaseData(test, err) {
		...
		const runComments = this.generateTestRunComments(test)

		if (jiraID.length && runComments.length) {
			let property = [{ property: { _attr: { name: 'test_key', value: jiraID } } },{ property: { _attr: { name: 'testrun_comment', value: runComments } } }]
			testcase.testcase.push({
				properties: property,
			})
		}
		else if (jiraID.length) {
	...
	generateJiraID(element) {
		...
		if (outdatedConfig) {
			console.log(
				"customised-xray-cypress-reporter v0.1.1 released: REPORTER OUTDATED CONFIG! Change the param 'env' with the value 'xray' to align the module with NEWEST CONFIGURATIONS"
			)
			if (!outdatedConfig) {
				return []
			}
	...
	generateTestRunComments(element) {
		const testConfig = element._testConfig
		if (!testConfig) {
			return []
		}
